...
True CREATE TABLE IF NOT EXISTS TCommande(idCommande int primary key auto_increment, Fproduit int, foreign key (Fproduit) REFERENCES TProduit(idProduit), Fclient int, foreign key (Fclient) references TClient(idClient), NbrProduit INT Not null, PrixTotal Double, DateCom date not null );
